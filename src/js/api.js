...
// mengupdate cerita
async function updateStory(id, formData, token = null) {
  try {
    console.log('Mengupdate cerita:', id);
    const res = await fetch(`${API_BASE}/stories/${id}`, {
      method: 'PUT',
      headers: token ? { 'Authorization': 'Bearer ' + token } : {},
      body: formData
    ...
    console.error('Terjadi error saat mengupdate cerita:', error);
    return { error: true, message: 'Gagal mengupdate cerita. Silakan cek koneksi internet Anda dan coba lagi.' };
  }
}

// menghapus cerita
async function deleteStory(id, token = null) {
  try {
    console.log('Menghapus cerita:', id);
    const res = await fetch(`${API_BASE}/stories/${id}`, {
      method: 'DELETE',
      headers: token ? { 'Authorization': 'Bearer ' + token } : {}
    });
    ...
    console.error('Terjadi error saat menghapus cerita:', error);
    return { error: true, message: 'Gagal menghapus cerita. Silakan cek koneksi internet Anda dan coba lagi.' };
  }
}
